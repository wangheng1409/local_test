...
for i in t:
    ...
    # if i=='all_yesterstoday':
    #     col = database.store_list.find({'latitude':{'$ne':None},'ts_string':{'$lte':str(yesterstoday)}}, {'id': 1})
    # else:
...
print('start_all_yesterday_yesterday')
col = database.store_list.find({'latitude': {'$ne': None}, 'ts_string': {'$lte': str(yesterstoday_yesterstoday)}},
                               {'id': 1})
col = [x for x in col]
print(col[0])
s = {}
...
l = len(list(s.keys()))
ss.append(l)
all_yesterday_yesterday_set = set(list(s.keys()))
...
item1 = ss[1]
item2 = ss[1] - ss[0]
item3 = len(list(yesterstoday_store_set - all_yesterday_yesterday_set))
item4 = category_num
item5 = ss[2]
