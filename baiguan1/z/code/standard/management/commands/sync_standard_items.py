...
class Command(BaseCommand):
    ...
    def load_from_trade_db(self):
        ...
        cur.execute(
            """
            SELECT item_id, item_name, price, device_id from trade_trade where item_id like '69%' and length(item_id) = 13 limit 1;
            """)
        result =  cur.fetchall()
        conn.commit()
        cur.close()
    ...
    def handle(self, *args, **options):
        ...
        for k in items:
            ...
            price = k[2]
            device_id = k[3]
            print '  %.2f) * Processing Item %s |||| %s' % (round(i/total*100.0, 2), barcode, item_name)
