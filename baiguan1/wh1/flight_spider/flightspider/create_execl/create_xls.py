...
def init_execl(company):
    ...
    res_sheet = workbook.add_sheet('%schange' % company)
...
def get_detail(company, res_sheet, workbook, dateFormat):
    ...
    for task in tasks:
        ...
        #        continue
        #dacode = '%s-%s' % (task["depCode"], task["arrCode"])
        ...
        if company == 'G5':
            try:
                ...
                continue
        #if eu_info is None and (company == "G5" or company == "EU"):
        ...
        # 是否提供常旅客积分
        res_sheet.write(line_num, 27, '否')
        # 证件类型
        res_sheet.write(line_num, 28, 1)
        # 29 预定office,可为空
        res_sheet.write(line_num, 29, 'PEK302')
        # 是否支持代码共享航班
        res_sheet.write(line_num, 30, '非共享')
        # 是否支持经停航班
        res_sheet.write(line_num, 31, '非经停')
        res_sheet.write(line_num, 32, cpa_type)
        res_sheet.write(line_num, 33, cpa_proportion)
        res_sheet.write(line_num, 34, 0)
        res_sheet.write(line_num, 35, 20)
        ...
        if line_num == 65536:
            line_num = 1
            sheet_num += 1
            res_sheet = workbook.add_sheet('%schange%d' % (company, sheet_num))
            pre_sheet(res_sheet)
    workbook.save(r'/data/download/%s_change.xls' % company.lower())


def create_zipfile(company):
    z = zipfile.ZipFile(r'/data/download/%s_change.zip' % company.lower(),
                        mode='w', compression=zipfile.zlib.DEFLATED)
    z.write(r'/data/download/%s_change.xls' % company.lower(),
            './%s_change.xls' % company.lower())
